...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    if (req.method === 'POST') {
        ...
        for (let image of images) {
          const { Url, error } = await storage.uploadFile(image);
          if (error) return res.status(400).json({ massage: "Some Thing Want Wrong", error: error });
          imagesUrls.push(Url);
        }
    
        const stripeImages = imagesUrls
        stripeImages.unshift(imageUrl);
