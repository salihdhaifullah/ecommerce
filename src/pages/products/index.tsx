...
export default function Index() {
  ...
  useEffect(() => {
    document.addEventListener("scroll", async () => { if (IsInView(LastElement) && !isLoadingRow) await HandelGetLast() })
  }, [])
        ...
        {isLoading ? <CircularProgress className="w-12 h-12" />
          : categoriesOptions.length > 0 ?
            <div className='flex gap-4 justify-center items-center flex-col'>
              <div className='flex overflow-auto justify-center items-center flex-col'>
                {categoriesOptions.map((category, index) => (
                  <Row category={category.name} key={index} />
                ))}
              </div>
              <div ref={LastElement} className="p-8 w-full flex justify-center items-center">{!isLoadingRow ? null : <CircularProgress className="w-8 h-8" />}</div>
            </div>
