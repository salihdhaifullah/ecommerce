...
const RowChild = ({ item, index }: IRowChild) => {
    ...
    return (
        <motion.div
            ...
            key={item.id}
            className='backdrop-blur-2xl w-fit bg-white min-h-[300px] min-w-[250px] flex flex-col rounded-lg p-4 my-12 hover:shadow-xl shadow-lg relative'>
            <div className="w-full flex flex-col items-center justify-between">
                ...
                <motion.div onClick={() => router.push(`/products/${item.id}`)} whileHover={{ scale: 1.25, rotateX: 25, rotate: -15 }} className='w-[200px] h-[200px] -mt-8 flex cursor-pointer'>
                    <Image width={200} height={200} src={item.imageUrl} alt={item.title} className='w-full h-full object-contain cursor-pointer' />
                </motion.div>
